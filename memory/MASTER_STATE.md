...
**Total items across 3 QA docs: 37. ✅ 27 · 🟡 1 · ❌ 7 · 🚧 2 · ❔ 0.** (Plus Bug #30 ✅.)
...
| G6 | fig 26 | Notes autosave | "Why is the platform not saving the notes made by the user in figure 26?" | "I think: The notes should be automatically saved in the background as the user enters or modifies content. Once the note is saved, display the date and time the note was last updated eg. Last updated: 2 June 2026, 10:45 AM. The existing note can be edited at any time and any changes made during editing should continue to be auto-saved. Users should be able to delete a note and a confirmation prompt should be displayed before deletion." | 🟡 SHIPPED tester-pending | Track B Phase B5 shipped 2026-06-04T07:36:00Z. `DocumentDrawer.NotesTab` rewritten: 1.0s debounced autosave via `useRef` race coalescer, `beforeunload` force-flush via `fetch(keepalive: true)`, `useEffect` cleanup force-flush via `api.patch`, "Last updated: {date}, {time}" indicator using `en-GB` long-month + `en-US` 12-hour formatter (matches spec "2 June 2026, 10:45 AM"), Delete-note via `window.confirm`. BE: new `documents.notes_updated_at` field set inside `patch_document` only when `body.notes is not None`; `sanitize_doc` passes `notes` + `notes_updated_at` through. 3 lockdown tests PASS (per-notes timestamp, delete path, debounce-flush idempotency). Z1.2 source-text test updated transparently (shorthand → explicit shape). |
...
- Phase B5 (Document workflow + Calendar polish): G6 (Notes autosave full spec) + G7 (Send Share Field-required error) + G10 (Calendar text in fig 32 to be removed) → 🟡 IN PROGRESS — G6 SHIPPED tester-pending 2026-06-04T07:36:00Z (`DocumentDrawer.NotesTab` rewrite + new `documents.notes_updated_at` field, 3 lockdown tests PASS, smoke screenshot clean). G7 + G10 NOT STARTED — separate dispatches per orchestrator ordering instruction. Memo: `sprints/TRACK_B_PHASE_B5_G6_NOTES_AUTOSAVE.md`.
...
Track B Phase B5 G6 (Notes autosave) shipped 2026-06-04T07:36:00Z. Awaiting tester journey-completion run on 6 numbered steps documented in `sprints/TRACK_B_PHASE_B5_G6_NOTES_AUTOSAVE.md`. G7 + G10 NOT STARTED — separate dispatches per orchestrator ordering.

---

## Section 7 — Last Updated

- **Written:** 2026-06-04T07:36:00Z (Track B Phase B5 G6 — Notes autosave per QA spec).
- **Agent:** track-b-phase-b5-g6-notes-autosave.
- **Mode:** Single-feature dispatch (smallest B5 item per credit-discipline). Four QA-spec behaviours implemented surgically: (1) 1.0s debounced autosave with in-flight race coalescing, (2) "Last updated: {date}, {time}" indicator backed by new `notes_updated_at` BE field, (3) continuous autosave on edit (free), (4) Delete-note via `window.confirm` (spec-meeting minimum, styled modal parked as future polish).
- **Files touched:** `M backend/routers/documents.py` (+12 LOC: per-notes timestamp on PATCH + `sanitize_doc` passthrough). `M frontend/src/components/documents/DocumentDrawer.jsx` (~225 LOC NotesTab rewrite + raw-fetch escape-hatch comment for `beforeunload` keepalive PATCH). `?? backend/tests/test_track_b_phase_b5_g6_notes_autosave.py` (3 lockdowns). `M backend/tests/test_sprint_z1_qa_fixes.py` (Z1.2 source-text test updated transparently — shorthand → explicit shape; PATCH contract unchanged).
- **Combined memo:** `sprints/TRACK_B_PHASE_B5_G6_NOTES_AUTOSAVE.md`
- **Regression:** **91/91 PASS in 13.12s** (G6 + Z1.2 updated + G11 + B3 + v1 byte-identical + Track A Phase 3 + workbook-analyze). ESLint clean on `DocumentDrawer.jsx`. Voice-lint clean.
- **Pre-handoff smoke:** CRA dev-server overlay confirmed gone (raw-fetch escape-hatch comment landed cleanly with Patch 24B convention reference). Zero React/compile errors. Legacy `[data-testid="drawer-notes-save"]` count = 0 (manual button removed). admin@akki.ai's preview workspace had no docs to drill into → live debounce/autosave round-trip is the tester journey.
- **Track B Phase B5 stays 🟡** until tester runs the 6-step journey.
