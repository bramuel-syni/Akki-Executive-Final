...
**Total items across 3 QA docs: 37. ✅ 28 · 🟡 1 · ❌ 6 · 🚧 2 · ❔ 0.** (Plus Bug #30 ✅.)
...
| G7 | fig 27, fig 28 | Send Share error | "Why does the error in figure 27 appear when the user clicks on 'Send Share' button in the same figure. The modal in figure 27 is pops up when user clicks 'share document' in figure 28?" | (Send Share should succeed without "Field required" false-positive) | 🟡 SHIPPED tester-pending | Track B Phase B5 G7 shipped 2026-06-04T07:59:00Z. Root cause: contract drift — FE always sent `recipients: [...]`, BE required singular `to_email: EmailStr` → every submit returned 422 "Field required". Fix: (a) FE rename `recipients` → `recipient_emails` (1 LOC); (b) BE schema swap to `recipient_emails: List[EmailStr] = Field(min_length=1, max_length=10)` mirroring Q4Y share at `questions.py:351`; (c) dual storage write (`recipient_emails` array + `shared_with_email` BC singular); (d) engagement-read surfaces array shape with BC fallback. 2 lockdowns / 4 sub-paths PASS. Live wire verified: happy path 200, legacy `recipients` payload 422 pointing at canonical field, engagement returns array. |
...
- Phase B5 (Document workflow + Calendar polish): G6 (Notes autosave full spec) + G7 (Send Share Field-required error) + G10 (Calendar text in fig 32 to be removed) → 🟡 IN PROGRESS (1/3 done + 1 tester-pending) — **G6 ✅ tester-verified 5/5 2026-06-04**. **G7 🟡 SHIPPED tester-pending 2026-06-04T07:59:00Z** (FE rename `recipients`→`recipient_emails` + BE schema swap + dual storage + engagement-read array shape; 2 lockdowns PASS; live wire verified happy path 200 + legacy payload 422). G10 NOT STARTED — final B5 dispatch after G7 tester PASS. Memos: `sprints/TRACK_B_PHASE_B5_G6_NOTES_AUTOSAVE.md`, `sprints/TRACK_B_PHASE_B5_G7_SEND_SHARE_VALIDATION.md`.
...
Track B Phase B5 G7 (Send Share "Field required" false positive) shipped 2026-06-04T07:59:00Z. Awaiting tester journey-completion run. G6 ✅ tester-verified 5/5; G10 NOT STARTED — final B5 dispatch after G7 tester PASS.

---

## Section 7 — Last Updated

- **Written:** 2026-06-04T07:59:00Z (Track B Phase B5 G7 — Send Share document endpoint contract alignment).
- **Agent:** track-b-phase-b5-g7-send-share-validation.
- **Mode:** Single-feature dispatch. Two-bug root cause: (1) BE schema required singular `to_email: EmailStr`; (2) FE always sent plural `recipients: [...]`. Every submit returned 422 "Field required". Fix aligned both sides to the canonical `recipient_emails: List[EmailStr]` shape that B3 Q4Y share + Pulse share + engagement-read already used.
- **Files touched:** `M frontend/src/components/documents/ShareDocumentModal.jsx` (1 LOC: `recipients` → `recipient_emails`). `M backend/routers/document_engagement.py` (+25 LOC: schema swap + dual-shape storage + engagement-read array + audit-log key rename). `?? backend/tests/test_track_b_phase_b5_g7_send_share_field_required.py` (2 lockdowns / 4 sub-paths).
- **Combined memo:** `sprints/TRACK_B_PHASE_B5_G7_SEND_SHARE_VALIDATION.md`
- **Live wire verified on preview (admin@akki.ai, in-browser fetch):** canonical `recipient_emails: ["alice@example.com","bob@example.com"]` → **HTTP 200** with fresh UUID; legacy `recipients: [...]` → 422 pointing at the new required field; engagement panel returns array `["alice@example.com","bob@example.com"]` + BC singular `shared_with_email = "alice@example.com"`. Smoke screenshot `/tmp/g7_smoke.png`.
- **Regression:** **93/93 PASS in 13.43s** (G7 + G6 + G11 + B3 + v1 byte-identical + Track A Phase 3 + workbook-analyze + Z1.2). ESLint clean. Ruff clean. Voice-lint clean.
