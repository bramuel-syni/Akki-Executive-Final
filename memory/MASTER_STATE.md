...
**Total items across 3 QA docs: 37. ✅ 28 · 🟡 0 · ❌ 7 · 🚧 2 · ❔ 0.** (Plus Bug #30 ✅.)
...
- Phase B5 (Document workflow + Calendar polish): G6 (Notes autosave full spec) + G7 (Send Share Field-required error) + G10 (Calendar text in fig 32 to be removed) → 🟡 IN PROGRESS (1/3 done) — **G6 ✅ SHIPPED tester-verified 5/5 2026-06-04** (`DocumentDrawer.NotesTab` rewrite + `notes_updated_at` field, 3 lockdowns PASS). G7 + G10 NOT STARTED — separate dispatches per orchestrator ordering instruction. Memo: `sprints/TRACK_B_PHASE_B5_G6_NOTES_AUTOSAVE.md`.
