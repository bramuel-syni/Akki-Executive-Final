...
|---|---|---|---|---|---|---|---|---|
| M.5 | Founding Cohort → Early access language swap | closed | • Hero secondary CTA: `Join the cohort` → `Request access`<br>• COHORT_TEASER headline/body/cta rewritten to "Akki is in early access" framing<br>• /cohort page title/description/submit/success rewritten<br>• COHORT block (kicker/dek/body/holding) swapped<br>• PRICING dek/footnote/table-header/CTA swapped<br>• AUDIENCE_PAGES Executive + NED pricing_line swapped<br>• CONTACT path label + body swapped<br>• Methodology prose recast<br>• WebsiteFooter link label swapped<br>• ForExco / ForNeds / ForExecutives primaryCta swapped<br>• Backend founder notify subject + body swapped<br>• Backend applicant_confirmation_body rewritten (≤80 words, voice-clean)<br>• `/early-access` → Navigate alias to `/cohort` (SPA-level)<br>• voice-lint `BANNED_PHRASES` regex (`founding cohort`, `join the cohort`)<br>• v1 byte-identical guard preserved<br>• Voice-lint clean | • `/cohort` route URL (SEO preserved)<br>• Mongo collection name `cohort_applications` (data integrity)<br>• POST `/api/cohort/applications` endpoint shape<br>• Founder notify recipients<br>• Form-field shape<br>• Audit event names (internal nomenclature)<br>• Backend `routers/website.py` (unrelated `/api/website/early-access` legacy endpoint — out of scope)<br>• Backend `services/cohort/*.py` internal class/function names<br>• Phase U.2 (paused) | `frontend/src/website/copy/index.js`, `frontend/src/website/pages/Cohort.jsx`, `frontend/src/website/pages/Pricing.jsx`, `frontend/src/website/pages/Methodology.jsx`, `frontend/src/website/pages/ForExco.jsx`, `frontend/src/website/pages/ForNeds.jsx`, `frontend/src/website/pages/ForExecutives.jsx`, `frontend/src/website/WebsiteFooter.jsx`, `frontend/src/App.js`, `backend/routers/cohort_applications.py`, `scripts/lint_voice.py` (BANNED_PHRASES infra) | `tests/test_phase_m5_early_access_swap.py` (24 tests · all surface lockdowns + alias route + lint phrase ban + regex behaviour) + 4 existing tests updated for new strings (`test_phase_m1_hero_rewrite`, `test_phase_m1x_cohort_teaser_alignment`, `test_phase_m2hold_cohort_hold`, `test_phase_m0c_cohort_applications`) + 2 M.1a recast assertions updated post-M.5 supersession (`senior` ban — M.1a's core invariant — still holds) | **Pytest:** Sprint M cumulative 153/153 GREEN, 2 skipped (pre-existing Phase Z-supersede). **Voice-lint:** clean across customer-copy surfaces with new phrase bans active. **v1 byte-identical guard:** 4/4 GREEN. **JS lint:** clean on every touched file. **Live multi-viewport raw DOM trace** at 1280×900 / 1024×768 / 820×1100 / 414×896 on `/` and `/cohort` → 8/8 probes verified new strings render verbatim AND old strings absent. `/early-access` alias trace: final URL = `/cohort`, new holding line + Request access CTA both visible. `/cohort` form submit e2e: created `Trace Co` request, success message `"Thank you. We have your request on file. We will share the offer with you before launch."` (verbatim). | 2026-02 |
| | NOTES — M.5 path-of-least-rework decision | — | **Phase U.2 (Microsoft OAuth) was in flight when this dispatch arrived; agent paused U.2 first.** Per the user directive: "finish Phase U.2 first if you've already started the implementation work, OR pause U.2 and do this swap first if you haven't. Use your judgement on whichever causes less rework." Justification for the pause: U.2 touches `frontend/src/components/auth/OAuthButtons.jsx` (sign-in surface) + new tests; M.5 touches `website/copy/index.js` / `website/pages/*` / `backend/routers/cohort_applications.py` + voice-lint + 5 test files. Zero file overlap, so swap-first = zero rework. U.2 resumes next dispatch with the same scope captured in this ledger's "In Progress" queue. | | | | |
