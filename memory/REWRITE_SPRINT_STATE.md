...
| D | Solva Backend Rewrite (UI unchanged) | **complete** (2026-05-16) | 1: framing-thin page from QA screenshot ("A COUPLE OF PIECES ARE THIN" leak) — structurally resolved by Phase D's silent Layer 0 + question-bank-driven Layer 1 opening. New collection `solva_phase_d_sessions`; new routes at `/api/contexts/{cid}/solva/v2/`. Legacy `routers/solva_v2.py` UNTOUCHED — Phase E will migrate the page. | `sprints/PHASE_D_CLOSEOUT.md` |
