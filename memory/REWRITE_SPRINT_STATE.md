...
| F.1 | Three production gaps (P0 + P1 + P2) | **complete** (2026-05-18) | 0 open. **P0** — Phase F Sub-task A anchoring fixed: dropped non-existent `account_id` query filter on `documents`, switched projection to real schema fields (`name`, `extracted_text`, `preview`), each anchor now carries an `excerpt` of the document body so FAR sees real Layer 0 evidence. **P1** — Mid-Solva-session document attach: new `POST /sessions/{sid}/attach-document` endpoint dispatched by Content-Type (multipart for new file, JSON for existing document_id), plus `GET /sessions/{sid}/attachments` listing view. Frontend paperclip on framing + Layer 1 + Layer 2 surfaces + `AttachDocumentModal.jsx` with Upload-new and From-Document-Journal tabs + inline confirmation + persistent anchor chips strip. **P2** — OCR + spreadsheet extraction: `.png/.jpg/.jpeg/.webp` via Tesseract+pytesseract, `.heic/.heif` via pillow_heif, `.xlsx` via openpyxl read-only, `.csv` via csv.reader. Per-image 5MB / 2400px downscale bound. Graceful failure on corrupted/empty images. 660 pytest passing (+12 net new). Render-smoke green. **Carry-over**: production Dockerfile needs `apt-get install -y tesseract-ocr` for the OCR path to be available in the deployed pod. | `sprints/PHASE_F1_CLOSEOUT.md` |
