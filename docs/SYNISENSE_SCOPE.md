...
- **ITEM D — Solve synthesis hook.** `routers/solve_engine.py:post_turn` runs Synisense on synthesis output BEFORE the Phase 11 validator runs. Validation now sees only redacted content (consistent with the "LLM never sees original" promise). `synisense_run_recorded` flag persisted on the synthesis record.
