...
| **Document Journal** (workspace listing + on-demand AI commentary) | **PARTIAL** | Workspace listing at `frontend/src/pages/Workspace.jsx` lists docs; row click opens viewer; "Open original" now opens an in-app modal (M.2 fix, applied today). `GET /api/contexts/{cid}/document-journal` and `POST /api/contexts/{cid}/documents/{did}/journal-commentary` are registered. **Caveat:** `db.documents` shows `journal_commentary` populated on **0 / 154** docs — the on-demand commentary endpoint exists but no row currently has been generated against it (no automated backfill has been run, so all commentary is generated lazily on first user click). |
