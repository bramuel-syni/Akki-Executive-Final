...
phase_c3:
  - task: "Phase C.3 — Work Studio: Solva session source path (from-source endpoint)"
    ...
    status_history:
        -working: false
          -agent: "testing"
          -comment: "BLOCKER on AC5a/AC5b/AC6 — POST /api/contexts/{cid}/work-studio/from-source returns 500 Internal Server Error. Root cause from /var/log/supervisor/backend.err.log: AttributeError: 'str' object has no attribute 'get' at /app/backend/work_studio/brief.py:127 in _bullets_from_recommendations -> `text = (r.get('text') or '').strip()`. The Solva session's recommendations list iterated by build_brief_from_solva() contains plain strings rather than {text:...} dicts, but the function unconditionally calls r.get(). Stack trace excerpt: File '/app/backend/routers/work_studio_from_source.py', line 233, in create_from_source -> build_brief_from_solva(...) -> File '/app/backend/work_studio/brief.py', line 264 -> _bullets_from_recommendations(recs) -> line 127 r.get('text'). SECONDARY non-fatal bug: same handler at work_studio_from_source.py line 298 logs 'audit write failed (non-fatal)' TypeError: write_audit() got an unexpected keyword argument 'target_id' — the audit-write helper signature has drifted from the call site (cosmetic, swallowed). FIX: in _bullets_from_recommendations, normalise: `text = (r.get('text') if isinstance(r, dict) else str(r) or '').strip()`. Then update write_audit kwargs (drop target_id or rename to entity_id/object_id per the helper's actual signature) to silence the non-fatal log line. AC RESULTS captured before the block: AC2=PASS (25 Solva sessions visible in inline-picker-solva_session, source-step-choices renders, c1-picker-grid + c1-format-pptx + c1-depth-board_summary + c1-fidelity-high all clickable), AC3=PASS (source-step-open + source-step-generate both visible simultaneously). AC5a=FAIL (POST /api/contexts/ec4db0c0-dea4-4ed6-81f2-da68994bfff2/work-studio/from-source -> 500; downstream POST /api/work_studio/exports never fires; no work-studio-export-complete shown). AC5b=FAIL (cascades from AC5a — no seeded brief, so no work-studio-row-brief-chip nor work-studio-row-refine on the Decks (in flight) tab). AC6 validated/refused=NOT REACHABLE. REGRESSION not exercised yet (browser-automation budget consumed). Network log: only POST /api/contexts/.../work-studio/from-source -> 500 fired; no /work_studio/exports POST; no /enhance POST. Fix the brief.py guard, then re-run the full Phase C.3 walkthrough."
