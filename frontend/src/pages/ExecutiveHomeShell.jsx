/**
 * LegacyAppHome — the pre-Phase-5 Home surface, kept byte-identical to
 * the v1 behaviour. Rendered whenever the ?home=v2 flag is absent or the
 * account is in sandbox mode. See /app/memory/HOME_AUDIT.md for the full
 * audit, and /app/docs/ux-advisories-v1.md (v1.5) for the rollout plan.
 */
...
export default function ExecutiveHomeShell() {
  ...
  return (
    <AppShell>
      ...
      <div className="min-h-[calc(100vh-4rem)] max-w-[1100px] mx-auto px-8 grid grid-cols-1 gap-10">
        {/* Phase 5 / v1.5 — tiny opt-in affordance for the new river-of-
            changes Home. Same playbook as ReadingView's ?v=2 flip. The
            link appends ?home=v2 so returning visitors can try the v2
            layout without a full rollout. Hidden on sandbox accounts
            (v2 is a no-op for them). */}
        {!account?.is_sandbox && (
          <div className="flex justify-end pt-4 pb-0" data-testid="home-v2-opt-in">
            <Link
              to="/app?home=v2"
              className="akki-overline tracking-[0.16em] text-[10.5px] text-[var(--muted)] hover:text-[var(--accent)] underline-offset-2 hover:underline"
            >
              Try the new home →
            </Link>
          </div>
